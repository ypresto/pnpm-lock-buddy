...
- `@pnpm/reviewing.dependencies-hierarchy` → `@pnpm/deps.inspection.tree-builder` (renamed upstream; `buildDependenciesHierarchy`/`PackageNode` → `buildDependenciesTree`/`DependencyNode`).
  ...
  - **`--depth` no longer limits duplicate detection, only path search**: the 0.06% depth-boundary diff above is now fully closed by decoupling the two. `traverseTreeAndBuildMap` and `collectFromTreeNodes` call `computeShallowestDepths` with `UNBOUNDED_TREE_DEPTH` (`tree-depth.ts`) unconditionally — detection is a BFS over an already-deduped, O(N) tree, so it's depth-unbounded-safe (unlike enumerating every *path* through a wide, heavily-shared graph, which can still blow up combinatorially within any fixed depth — the reason `--depth` still caps `findPathInTree`/`findAllPathsInTree`, i.e. `--deps`). `DuplicatesUsecase` no longer keeps its own `depth` field; the constructor parameter is forwarded to `DependencyTracker` for path search only. Re-verified on the same 135-project monorepo: duplicate-package count rose to 23663 (above the per-project fix's 23649 — detection is now genuinely exhaustive rather than matching that setup's depth-10 cap by coincidence), the previously-missed packages are now found, and there was no measurable performance cost (~2.3s, same ballpark as before this change).
