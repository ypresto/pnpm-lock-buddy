...
- `@pnpm/reviewing.dependencies-hierarchy` → `@pnpm/deps.inspection.tree-builder` (renamed upstream; `buildDependenciesHierarchy`/`PackageNode` → `buildDependenciesTree`/`DependencyNode`).
  ...
  - **Known issue, found via real-world testing against a 135-project production monorepo, NOT YET FIXED**: the renamed library also bounds a whole-workspace tree build to O(N) nodes by returning every repeat occurrence of an already-expanded subtree as an empty `deduped: true` stub. This codebase's traversal didn't know about that contract and silently treated every stub as a childless leaf — on the test monorepo, 41% of all nodes came back deduped, and duplicate detection dropped from 123 to 53 affected projects (a false ~8x speedup that was actually a correctness bug). A first fix attempt (`src/core/tree-dedup.ts`, resolving each stub by matching `path`+`peersSuffixHash` against another occurrence) made this worse, not better: re-verification against the same monorepo found it attributing one project's dependencies to a different, unrelated project (duplicate-package count went from a true 10019 to a fabricated 22958). Root cause, confirmed by reading the library's source (`getTree.js`): its real materialization-cache key is `(graph nodeId, remaining tree depth)`, neither of which is exposed on the public `DependencyNode` shape a fix could match against, so no external heuristic on that shape can safely reconstruct a deduped stub's real children when the cache spans multiple unrelated projects. Being reworked to scope each project's `buildDependenciesTree` call independently instead (eliminating cross-project cache sharing structurally, at the cost of one lockfile read per project instead of one for the whole workspace).
